...
//        notifyObservers(sipRequest);
    ...
    public ClientTransaction preProcessInvite(SipRequest sipRequest)
        ...
        String uaString = this.userAgent.getConfig().getUserAgentString();
        if(uaString != null ) {
            requestHeaders.add(new SipHeaderFieldName(RFC3261.HRD_USER_AGENT),
                    new SipHeaderFieldValue(uaString));
        }
